...
buy one and sell one share of the stock multiple times) with 
...
// Four states are used for the dp: buy, sell, coolDown and noOp,
// where noOp happens between buy and sell, coolDown happens between
// sell and buy.
// 
...
//
// Then the update works as buy[i] = coolDown[i-1]-prices[i],
// coolDown[i] = max(coolDown[i-1], sell[i-1]), noOp[i] = max[noOp[i-1],
// buy[i-1]]] and sell[i] = max(noOp[i-1], buy[i-1]) + prices[i].
// 
// The constant space solution readily follows this since current states
// for price i only depends on previous states for price i-1.
