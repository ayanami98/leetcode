 ...
 *              indexed 1 or the project indexed 2. Since you can choose at most
 *              2 projects, you need to finish the project indexed 2 to get the
...
public:
    int findMaximizedCapital(int k, int W, vector<int>& Profits, vector<int>& Capitals) {
        ...
        for (int i = 0; i < Profits.size(); ++i) {
            pq.emplace(Capitals[i], Profits[i]);
        }
        int capital = W;
        priority_queue<int> profits;
        while (k-- > 0) {
            while (!pq.empty() && pq.top().first <= capital) {
                profits.push(pq.top().second);
                pq.pop();
            }
            if (profits.empty()) {
                break;
